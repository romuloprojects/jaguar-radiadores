    ...
    <InternalPage>
      ...
      <div className="grid min-w-0 gap-4">
        ...
        {selected ? <aside className="panel customer-detail-panel">
          <div className="customer-detail-panel__header"><div className="customer-mark">{selected.kind==="PJ"?<Building2/>:<UserRound/>}</div><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h2>{selected.name}</h2><StatusPill label="Ativo" tone="positive"/></div><p>Cód. {selected.id.slice(0,8)}</p></div><Button asChild variant="outline" size="sm" className="ml-auto"><Link to="/clientes/$clienteId" params={{clienteId:selected.id}}><Pencil className="mr-2 h-3.5 w-3.5"/>Editar</Link></Button></div>
