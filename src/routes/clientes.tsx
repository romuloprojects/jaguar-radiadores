...
function CustomersPage() {
  ...
  const clients = useQuery({ queryKey: ["clients"], queryFn: () => jaguarApi.clients.list({ limit: 500 }) });
  const filtered = useMemo(() => {
    const needle = query.trim().toLocaleLowerCase("pt-BR");
    return (clients.data?.items ?? []).filter((c) => {
      if (kind !== "ALL" && c.kind !== kind) return false;
      if (!needle) return true;
      return [c.name, c.phone, c.document, c.responsible, c.city].filter(Boolean).some((value) => String(value).toLocaleLowerCase("pt-BR").includes(needle));
    });
  }, [clients.data, kind, query]);
...
function NewCustomerDialog() {
  ...
  const mutation=useMutation({mutationFn:()=>jaguarApi.clients.create({...form,kind}),onSuccess:()=>{toast.success("Cliente cadastrado.");setOpen(false);setForm({name:"",document:"",phone:"",responsible:"",street:"",city:"",state:"PR"});silentInvalidate(qc,[["clients"]]);},onError:(e)=>toast.error(e.message)});
