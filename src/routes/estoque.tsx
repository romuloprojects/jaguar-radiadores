...
const recentMoves = [
  {
    id: 1,
    kind: "Entrada",
    description: "Colmeia caminhão linha pesada",
    amount: "+ 4 un.",
    date: "23/09/2026 14:32",
  },
  {
    id: 2,
    kind: "Saída",
    description: "Conexão superior 45 mm",
    amount: "- 2 un.",
    date: "23/09/2026 11:18",
  },
  {
    id: 3,
    kind: "Ajuste",
    description: 'Mangueira reforçada 2"',
    amount: "+ 2 un.",
    date: "22/09/2026 16:45",
  },
  {
    id: 4,
    kind: "Reserva",
    description: "Tampa de radiador 1.1 bar",
    amount: "- 1 un.",
    date: "22/09/2026 10:27",
  },
];
...
function InventoryPage() {
  ...
  const filtered = useMemo(
    () =>
      stockItems.filter((i) =>
        `${i.code} ${i.description} ${i.category} ${i.supplier}`
          .toLowerCase()
  ...
  return (
    <InternalPage>
      <PageHeader
        title="Estoque"
        subtitle="Controle de produtos, peças, saldos e movimentações da Jaguar Radiadores."
        right={
          <Button>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          ...
          icon={Boxes}
          accent="red"
          detail="Catálogo de peças e consumíveis"
        />
        <StatCard
          label="Estoque crítico"
          value="29"
          detail="Reposição prioritária"
          icon={AlertTriangle}
          accent="red"
        />
        <StatCard
          label="Valor em estoque"
          value={brl(inventoryValue * 18.5)}
          icon={Warehouse}
          accent="green"
        ...
        <StatCard
          ...
          value={stockItems.reduce((s, i) => s + i.reserved, 0)}
          detail="Ligados a atendimentos"
          icon={PackageSearch}
          accent="graphite"
        />
      </div>
        ...
        <div className="space-y-3 min-w-0">
          <FilterBar>
            <div className="flex flex-col gap-3 lg:flex-row">
              ...
              <Button variant="outline">Todos os fornecedores</Button>
                  ...
                  <tr key={item.id}>
                    <td>
                      {item.imageUrl ? (
                        ...
                        <div
                          className="inventory-photo-slot"
                          title="Espaço reservado para foto real do item"
                        >
                    ...
                    <td>
                      <b className="block text-foreground">{item.description}</b>
                      <small className="text-muted-foreground">{item.supplier}</small>
                    </td>
                    <td>{item.category}</td>
                    <td>{item.current}</td>
                    <td>{item.reserved}</td>
                    <td
                      className={
                        item.current - item.reserved <= item.minimum
                          ? "font-bold text-[var(--accent-red)]"
                          : "font-bold text-[var(--accent-green)]"
                      }
                    >
                      {item.current - item.reserved}
                    </td>
                    <td>{brl(item.unitCost)}</td>
                    <td>{brl(item.salePrice)}</td>
                    <td>
                      <StatusPill
                        label={item.status}
                        tone={
        ...
        <aside className="grid gap-4 lg:grid-cols-2">
          <section className="panel inventory-side-card">
            ...
            <div className="inventory-donut">
              ...
              <div className="inventory-donut__center">
                <b>435</b>
                <span>itens</span>
              </div>
            </div>
          ...
          <section className="panel inventory-side-card">
            ...
            <div className="mt-3 space-y-1">
              {recentMoves.map((move) => (
                <div className="stock-move" key={move.id}>
                  <span
                    className={
                      move.kind === "Entrada" || move.kind === "Ajuste" ? "positive" : "negative"
                    }
                  >
                    {move.kind === "Entrada" || move.kind === "Ajuste" ? (
                      <ArrowDown />
                    ) : (
                      <ArrowUp />
                    )}
