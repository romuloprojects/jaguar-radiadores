...
function CustomerDetailPage() {
  ...
  const save=useMutation({mutationFn:()=>jaguarApi.clients.update(form),onSuccess:async()=>{toast.success("Cliente atualizado.");await qc.invalidateQueries({queryKey:["client",clienteId]});await qc.invalidateQueries({queryKey:["clients"]});},onError:e=>toast.error(e.message)});
...
function VehicleDialog({customerId,vehicle}:{customerId:string;vehicle?:VehicleApi}){
  ...
  const mutation=useMutation({mutationFn:()=>vehicle?jaguarApi.clients.updateVehicle({id:vehicle.id,...form}):jaguarApi.clients.createVehicle({customerId,...form}),onSuccess:async()=>{toast.success(vehicle?"Veículo/equipamento atualizado.":"Veículo/equipamento adicionado.");setOpen(false);await qc.invalidateQueries({queryKey:["client",customerId]});},onError:e=>toast.error(e.message)});
