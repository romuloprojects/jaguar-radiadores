...
const ACCENT_VAR: Record<AccentName, string> = {
  ...
  graphite: "var(--accent-graphite)",
};

export function InternalPage({ children, className }: { children: ReactNode; className?: string }) {
  return <div className={cn("internal-page space-y-4", className)}>{children}</div>;
}
...
export function SectionPanel({
  ...
  return (
    <section className={cn("panel section-panel overflow-hidden", className)}>
      ...
      <div className={cn("section-panel__content", contentClassName)}>{children}</div>
    </section>
  );
}
...
export function StatCard({
  ...
  return (
    <div className={cn("panel summary-card", className)}>
      <div className="summary-card__layout">
        {Icon && (
          <span
...
export function FilterBar({ children, className }: { children: ReactNode; className?: string }) {
  return <div className={cn("panel page-toolbar", className)}>{children}</div>;
}
